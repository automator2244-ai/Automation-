...
| **הגנת clickjacking (זמנית)** | אם מישהו מטמיע את האתר ב-iframe, התוכן מוסתר לחלוטין — כך אי אפשר להשתמש בדף כשכבה מזויפת לגניבת לידים. זהו פתרון JS זמני; הפתרון האמיתי הוא header, בשלב 2. |
| **הצלת לידים בכשל** | אם השליחה נכשלת, במקום שהליד ילך לאיבוד מוצג כפתור וואטסאפ עם כל הפרטים שהמבקר כבר מילא, מוכן לשליחה. |
...
**זה שלב קריטי שאסור לדלג עליו — אבל הוא חייב להיות אחרון.**
...
> 🛑 **סדר הפעולות קריטי.** הדף החי מצביע כרגע על ה-webhook **הישן**. אם תחליף את הטריגר בסנריו ל-webhook החדש לפני שה-Worker באוויר והדף מחובר אליו — **הטופס באתר יפסיק להביא לידים באותו רגע.**
>
> אם כבר יצרת webhook חדש: מצוין, פשוט **אל תחבר אותו לסנריו עדיין** ואל תמחק את הישן. השאר את הסנריו על ה-webhook הישן עד שנסיים.

הרוטציה עצמה, רק אחרי ששלב 6 הושלם ואומת:

1. `wrangler secret put MAKE_WEBHOOK_URL` — הפעם עם הכתובת **החדשה**
2. ב-Make: פתח את הסנריו → לחץ על המודול הראשון (Custom webhook) → בחר ברשימה את ה-webhook החדש → Save
3. לחץ **Redetermine data structure** ושלח ליד בדיקה מהאתר, כדי ש-Make ילמד מחדש את מבנה הנתונים. שים לב: מודולים שאחרי הטריגר עלולים לאבד את המיפוי ולהציג סימני קריאה אדומים — צריך למפות אותם מחדש. ה-payload מה-Worker כולל שדה נוסף בשם `ip`; שאר השדות זהים.
4. שלח ליד בדיקה אמיתי וודא שהוא הגיע
5. **רק עכשיו** מחק את ה-webhook הישן
